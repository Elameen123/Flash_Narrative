...
# --- NEW: Gemini API Client (FOR BATCH SENTIMENT) ---
...
# --- Schema for Batch Sentiment Analysis (FOR GEMINI) ---
SENTIMENT_SCHEMA = {
    "type": "ARRAY",
    "items": {
        "type": "OBJECT",
...
# --- Batch Sentiment Function (FOR GEMINI) ---
...
# --- Bedrock Client Setup (FOR AI SUMMARY) ---
@st.cache_resource
...
# --- Model Configuration (FOR AI SUMMARY) ---
...
# --- Helper Functions for Bedrock Model Payloads (FOR AI SUMMARY) ---

def _build_anthropic_body(prompt, max_tokens=1000, temperature=0.6):
...
def _build_meta_llama_body(prompt, max_tokens=1000, temperature=0.6):
...
def _build_amazon_titan_body(prompt, max_tokens=1000, temperature=0.6):
...
def _build_cohere_body(prompt, max_tokens=1000, temperature=0.6):
...
def _build_mistral_body(prompt, max_tokens=1000, temperature=0.6):
...
# --- Bedrock API Call Function with Fallback (FOR AI SUMMARY) ---
...
# --- Original (single) Sentiment Function (NOT USED, but kept for reference) ---
def get_llm_sentiment(text_chunk):
    ...
    (This function is no longer called by dashboard.py)
    """
...
# --- UPDATED Report Summary Function (NOW USES BEDROCK) ---
...
"""
    ...
    # --- Call the Sequential Invoker (NOW USES BEDROCK) ---
    result_text = invoke_model_sequentially(
        prompt=prompt,
    ...
    if result_text:
        ...
        return ("**Error:** Could not generate AI Report Summary using available Bedrock models.\n"
