...
custom_css = f"""
<style>
    /* (All CSS rules are the same as your original file) */
...
def run_analysis(brand, time_range_text, hours, competitors, industry, campaign_messages):
    ...
    try:
        ...
        # 2. --- NEW: Batch Sentiment Analysis with Fallback ---
        st.write("🧠 Performing Batch Sentiment Analysis...")
        temp_data = scraped_data['full_data']
        progress_bar = st.progress(0, text="Preparing batch analysis...")

        # Create items with unique IDs for batching
        items_to_analyze = [
            {"id": i, "text": item.get("text", "")}
            for i, item in enumerate(temp_data)
        ]

        sentiment_map = {}
        if items_to_analyze:
            with st.spinner(f"Analyzing {len(items_to_analyze)} items with Gemini (this may take a moment)..."):
                # Call the new batch function from bedrock.py (aliased as bedrock_llm)
                sentiment_map = bedrock_llm.get_batch_llm_sentiments(items_to_analyze)

        progress_bar.progress(0.5, text="Assigning sentiments...")
        llm_failed_count = 0
        total_items = len(temp_data)

        for i, item in enumerate(temp_data):
            # Look up the sentiment by id (which is the index i)
            llm_sentiment = sentiment_map.get(i)

            # Fallback to keyword analysis if Gemini failed for this specific item
            ...
            progress_text = f"Assigning sentiments ({progress_percent * 100:.0f}%)..."
        ...
        if llm_failed_count > 0:
            st.warning(f"⚠️ AI analysis failed or returned no sentiment for {llm_failed_count}/{total_items} items. Used keyword fallback for those items.")
        elif not items_to_analyze:
            st.warning("No text found to analyze.")
        else:
            st.success("Batch sentiment analysis complete!")
        # --- END OF NEW SENTIMENT SECTION ---
        ...
        # 4. Extract Keywords/Phrases (No change)
        ...
        if hasattr(analysis, 'stop_words') and isinstance(analysis.stop_words, set):
            current_stop_words = analysis.stop_words.copy()
        ...
        if neg_pct > 30: # Threshold for alerts
            ...
            if alert_email:
                ...
                servicenow_integration.send_alert(alert_msg, channel='#alerts')
...
def display_dashboard(brand, competitors, time_range_text, thresholds):
    ...
    # (This entire function remains unchanged from your original file)
    ...
    if len(sov_values) != len(all_brands):
        ...
        for item in st.session_state.full_data:
             ...
             if present_brands_in_item:
                 relevant_mentions_count +=1
                 for b in present_brands_in_item: brand_counts[b] += 1
        ...
        st.session_state.kpis['sov'] = sov_values
    ...
    if st.button("Generate Reports for Email/Download", use_container_width=True, key="generate_reports"):
        if not st.session_state.kpis or not st.session_state.full_data:
            ...
            with st.spinner("Building PDF report (using Gemini for summary)..."):
                try:
                    # This now calls the Gemini-powered summary function
                    ai_summary = bedrock_llm.generate_llm_report_summary(st.session_state.kpis, st.session_state.top_keywords, st.session_state.full_data, brand, competitors)
            ...
            if not email_to_send: st.button("Email Reports", disabled=True, use_container_width=True, help="Enter email above.")
                ...
                with st.spinner(f"Sending to {email_to_send}..."):
                    ...
                    if sent:
                        ...
                        st.success(f"Emailed to {email_to_send}!")
                    else:
...
def main():
    ...
    # (Sidebar and KPI Threshold inputs remain the same)
    with st.sidebar:
    ...
    # (Main dashboard inputs remain the same)
